...
> **2026 context.** Software engineering has been the fastest-moving function. **GitHub Copilot**, **Cursor**, and **Windsurf** (Codeium) now offer agentic coding — planning, editing multiple files, running tests, and opening PRs. Claude's ** agentic coding** via the Agent SDK and Cursor has narrowed the gap with closed tools. The productivity gains described here have compounded: agentic coding assistants now handle 30–50% of routine implementation work. For the agentic layer, see [Orchestration Frameworks](/docs/genai-playbook/agent-orchestration-frameworks/).
